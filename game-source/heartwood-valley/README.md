...
- Talk and give a gift once per neighbor per day. Reach two hearts to date. All eight adults can be partners simultaneously.
...
- **Crombot** waits on the main road just right of the shop. His original transparent Cromblog cutout and new pixel portrait preserve the real Crombot’s cardboard chassis, sensor eyes, wheels, and wire crown. He loves sunflowers, has a **Run flirt protocol** button for repeatable hardware innuendo, and a moonlit picnic heart event. Flirt-protocol jokes do not bypass daily affection limits.
- With two or more partners, host the long-table picnic. Dating all eight completes the final journal milestone.
...
Existing six- and seven-neighbor saves automatically gain any missing goblin and Crombot bonds while retaining crops, resources, dates, and other progress.
...
- `public/assets/`: original AI-generated village, portraits, character sprite sheet, item atlas, and dedicated goblin and Crombot sprites and portraits.
...
This intentionally small demo uses one map, twelve beds, eight neighbors, three crops, and one fish. Character sprites, selected tools, crops, and action feedback render on canvas. Music begins only after enabling it and pauses when the tab is hidden. The date choices share the same positive relationship outcome. The parody uses original art and music; no Stardew Valley assets are included.
